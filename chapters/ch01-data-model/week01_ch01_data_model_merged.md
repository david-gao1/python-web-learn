...
### 2.4 为什么“通常不直接调用 dunder 方法”

你当然可以直接写 `book.__len__()`、`book.__iter__()`，但工程里通常不这么写，原因很现实：
- 可读性：`len(book)`、`for x in book` 是 Python 社区约定写法，团队一眼能看懂。
- 语义一致：同一个入口可以作用于不同类型（内置类型 + 自定义类型），减少“特殊情况”。
- 让解释器做正确的路由：例如 `x in container` 会优先走 `__contains__`，没有才退化为迭代扫描；你手动调某个 dunder 可能跳过回退语义。

所以更推荐的心智模型是：你实现 dunder，是为了让解释器/标准库在“语法点”自动调用它们。
...
### 3.0 `@dataclass(frozen=True)`：让领域对象更“像数据”
...
它的含义与作用：
- `@dataclass`：由字段声明自动生成 `__init__`、`__repr__`、比较方法等样板代码，让类更专注“数据结构”。
- `frozen=True`：实例创建后字段不可修改，能更稳地维护不变量（订单金额、数量不应该被随手改掉）。

工程上你可以把它理解成：把 `Order` 变成“可安全传递的值对象（value object）”，降低被误改的概率。
